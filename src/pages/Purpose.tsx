...
import { Trash2, Edit, Plus, RefreshCcw, Search } from "lucide-react"
...
export default function PurposePage() {
  ...
  // delete modal hook
  const { isDeleteModalOpen, itemToDelete, openDeleteModal, closeDeleteModal } =
    useDeleteModal<Purpose>()
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, title }: { id: string; title: string }) =>
      apiService.updatePurpose(id, { title }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["purposes"] })
      setModalOpen(false)
      setEditing(null)
  ...
  return (
    <>
                    ...
                    purposes.map((purpose: Purpose) => (
                      <tr key={purpose._id} className="border-b">
                        ...
                        <td className="py-2 text-right space-x-2">
                          ...
                          >
                            <Trash2 size={20} className=" text-red-600 hover:text-red-900" />
                          </Button>
                        </td>
                      </tr>
                    ))
      ...
      <ReusableDeleteModal
        ...
        // itemName={itemToDelete?.title || "this purpose"}
