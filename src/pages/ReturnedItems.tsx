...
const PurchaseModal: React.FC<PurchaseModalProps> = ({ isOpen, onClose, purchase }) => {
  ...
  const { data: purchasesData } = useQuery({
    ...
    queryFn: () => apiService.getPurchases({ all: true }),
    refetchOnMount: true,
    refetchOnWindowFocus: true,
    staleTime: 0,
  });
  ...
  const {
    ...
    queryKey: ['returns', purchase?._id],
    queryFn: async () => {
      if (!purchase?._id) return null;
      return apiService.getPurchaseReturnById(purchase._id);
    },
  ...
  useEffect(() => {
    if (!selectedRefNum || !filteredPurchases?.length) return;

    const matchedPO = filteredPurchases?.find(
    ...
    if (matchedPO) {
      ...
      reset((prevValues: any) => ({
        ...
        receivedBy: matchedPO?.receivedBy,
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      if (isEditing && currentPurchase && currentPurchase._id) {
        return apiService.updatePurchaseReturn(currentPurchase._id, formData);
      } else {
        return apiService.createPurchaseReturn(formData);
      }
    },
    onSuccess: () => {
...
export const ReturnedItems: React.FC = () => {
  ...
  const { data: purchasesData, isLoading, refetch } = useQuery<PurchasesApiResponse>({
    queryKey: ['purchase_returns', page, debouncedSearch],
    queryFn: () => apiService.getPurchaseReturns({ page, limit: 10, search: debouncedSearch }),
    refetchOnMount: true,
    refetchOnWindowFocus: true,
    staleTime: 0,
  });

  const purchases = purchasesData?.returns || [];
  const pagination = purchasesData?.pagination || { page: 1, pages: 1, total: 0, limit: 10 };

  const filteredPurchases = purchases
  console.log("Purchase Return", filteredPurchases)


  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiService.deletePurchaseReturn(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchase_returns'] });
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader
            ...
            subtitle="Create and manage your Purchase Returns"
            action={
              <Button
                icon={Plus}
                ...
                {filteredPurchases.map((purchase, index) => (
                  <tr key={purchase._id} className={`hover:bg-gray-50`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      ...
                      <button
                        onClick={() => navigate(`/purchase-return/${purchase._id}`)}
      ...
      <ReusableDeleteModal
        ...
        message="Are you sure you want to delete purchase return"
